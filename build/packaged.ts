...
function main() {
	...
	const iframeHtmlPath = path.join(rootDir, 'iframe/index.html');
	const iframeScriptPath = path.join(rootDir, 'dist/iframe/main.js');
	const iframeHtml = fs.readFileSync(iframeHtmlPath, 'utf-8');
	const iframeScript = fs.readFileSync(iframeScriptPath, 'utf-8');
	const inlineIframeHtml = iframeHtml.replace(
		'<script src="/dist/iframe/main.js"></script>',
		`<script>${iframeScript}</script>`,
	);
	fs.writeFileSync(iframeHtmlPath, inlineIframeHtml, 'utf-8');

	packageExtension(rootDir, outputPath).then(() => {
		console.log(`Packaging complete: ${outputPath}`);
		fs.writeFileSync(iframeHtmlPath, iframeHtml, 'utf-8');
	}).catch((err) => {
